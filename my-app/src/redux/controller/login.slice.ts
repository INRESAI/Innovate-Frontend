...
const loginSlice = createSlice({
    ...
    reducers: {
        ...
        getCurrentUserByTokenRequest(state, action: PayloadAction<any>){
            state.loading = true

        },
        getCurrentUserByTokenSuccess(state,action: PayloadAction<any>){
...
const getCurrentUserByToken$: RootEpic = (action$) => action$.pipe(
    filter(getCurrentUserByTokenRequest.match),
    switchMap((re) => {
        
        return IdentityApi.getCurrentUserByToken(re.payload).pipe(
            mergeMap((res: any) => {
                    console.log(res);
                    // console.log(res.data.accessToken);
                    // const token = res.data.accessToken
                    ...
                    console.log(user);
                    return [
                        loginSlice.actions.getCurrentUserByTokenSuccess(user)
                    ];
            }),
            catchError(err => [loginSlice.actions.loginFail(err)])
        )
